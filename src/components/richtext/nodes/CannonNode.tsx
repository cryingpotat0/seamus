...
export class CannonNode extends DecoratorNode<JSX.Element> {
  ...
  decorate(): JSX.Element {
    ...
    const editorExtensions = [
      getLanguageExtension(this.__props.languageProps.language),
      createTheme(theme),
    ];

    return (
      <Suspense fallback={null}>
        <CannonProvider {...this.__props} allowBuilder={true}>
          <SyncSerializeFn obj={this} fnName={"serialize"} />
          <CodeEditor extensions={editorExtensions} theme={theme} />
          <Terminal
            config={{
              theme,
            }}
...
export function $createCannonNode(
  serializedNode?: CannonSerializedProps
): CannonNode {
