...
import {
  $createJupyterHtmlNode,
  JupyterHtmlNode,
} from "../../nodes/JupyterHtmlNode";

export const INSERT_JUPYTER_HTML_COMMAND: LexicalCommand<string> =
  createCommand("INSERT_JUPYTER_HTML_COMMAND");
...
export default function JupyterHtmlPlugin(): JSX.Element | null {
  ...
  useEffect(() => {
    if (!editor.hasNodes([JupyterHtmlNode])) {
      throw new Error(
        "JupyterHtmlPlugin: JupyterHtmlNode not registered on editor"
      );
    }
